...
const SalesOverview = () => {
  ...
  const fetchSales = async () => {
    try {
      ...
      const uniqueClerks = [...new Set(salesResponse.data.map(sale => sale.clerkName))];
  ...
  const applyFilters = () => {
    ...
    // Apply date filters
    if (filters.dateFrom) {
      filtered = filtered.filter(sale => new Date(sale.date) >= new Date(filters.dateFrom));
    }
    
    if (filters.dateTo) {
      filtered = filtered.filter(sale => new Date(sale.date) <= new Date(filters.dateTo));
    }
    
    // Apply clerk filter
    if (filters.clerk) {
      filtered = filtered.filter(sale => sale.clerkName === filters.clerk);
    }
                    ...
                    {filteredSales.length > 0 ? (
                      filteredSales.map((sale) => (
                        <tr key={sale.id} className="hover:bg-emerald-50 transition-all duration-200">
                          ...
                          <td className="px-6 py-4 text-emerald-700 text-sm">{new Date(sale.date).toLocaleDateString()}</td>
                          <td className="px-6 py-4 text-emerald-700 text-sm">{sale.clerkName}</td>
                          <td className="px-6 py-4 text-emerald-700 text-sm">{sale.paymentMethod}</td>
                          <td className="px-6 py-4 text-right text-emerald-900 font-semibold">${parseFloat(sale.totalAmount).toFixed(2)}</td>
              ...
              <div className="border border-emerald-200 rounded-lg p-6 mb-6">
                <div className="grid grid-cols-2 gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{selectedInvoice.clerkName}</p>
                  </div>
                  <div>
