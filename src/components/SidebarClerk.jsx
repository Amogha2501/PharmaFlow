...
import { X, LayoutDashboard, Receipt, Package, History } from 'lucide-react';

const SidebarClerk = ({ isOpen, onClose }) => {
  const location = useLocation();
  
  const menuItems = [
    { icon: LayoutDashboard, label: "Dashboard", path: "/dashboard/clerk" },
    { icon: Receipt, label: "Sales POS", path: "/dashboard/clerk/sales" },
    { icon: Package, label: "Inventory View", path: "/dashboard/clerk/inventory" },
    { icon: History, label: "Transaction History", path: "/dashboard/clerk/history" },
  ];
        ...
        <div className="flex items-center justify-between p-4 border-b border-emerald-700 bg-emerald-800/50">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-emerald-200 truncate">Clerk Dashboard</p>
            </div>
          </div>
          <button 
            onClick={onClose}
